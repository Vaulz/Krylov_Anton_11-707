    ...
    public class BigInteger
    {
        ...
        class Program
        {
            static void Main()
            {
                BigInteger n = "5";
                BigInteger k = "8";
                BigInteger firstMemberofSequence = "-80";
                var sum = GetSumOfFirstNTerms(n, k, firstMemberofSequence);
                Console.WriteLine(sum);
            }

            static BigInteger GetSumOfFirstNTerms(BigInteger n, BigInteger k, BigInteger firstMemberofSequence)
            {
                return ((2 * firstMemberofSequence + (n - 1) * k) * n) / 2;
            }
        }
    }
}